...
import { drawTrajectories, drawSlowMotionBanner } from '../game/slow-motion-draw';
...
export default function GameCanvas() {
  ...
  const slowMotionHitBallIds = useGameStore((s) => s.slowMotion.hitBallIds);
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      const curSlowActive = state.slowMotion.active && !state.slowMotion.finished;
  ...
  const draw = (ctx: CanvasRenderingContext2D, curBalls: Ball[], t: Table) => {
    ...
    if (slowMotionActive) {
      drawTrajectories(ctx, slowMotionFrames, slowMotionFrameIndex, slowMotionHitBallIds);
    }
